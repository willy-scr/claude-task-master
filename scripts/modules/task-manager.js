/**
 * task-manager.js
 * Task management functions for Task Master CLI
 */
...
import { 
  ...
  analyzeTaskComplexity as analyzeComplexityWithGemini,
  geminiClient
} from './ai-services.js';
...
async function expandTask(taskId, numSubtasks = CONFIG.defaultSubtasks, useResearch = false, additionalContext = '') {
  try {
    ...
    if (useResearch) {
      ...
      subtasks = await createSubtasksWithGemini(task, numSubtasks, nextSubtaskId, additionalContext);
    } else {
      log('info', 'Generating subtasks with Gemini only');
      subtasks = await createSubtasksWithGemini(task, numSubtasks, additionalContext);
    }
  ...
  try {
    ...
    for (const task of tasksToExpand) {
      try {
        ...
        let subtasks;
        if (useResearch) {
          subtasks = await createSubtasksWithGemini(task, taskSubtasks, taskContext);
        } else {
          subtasks = await createSubtasksWithGemini(task, taskSubtasks, taskContext);
        }
...
Respond only with valid JSON for the following tasks.
...
${prompt}
          ...
          console.error(chalk.red(`Error using Gemini for complexity analysis: ${error.message}`));
          throw error;
        } finally {
          if (streamingInterval) clearInterval(streamingInterval);
          stopLoadingIndicator(loadingIndicator);
