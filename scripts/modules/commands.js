...
import {
  addDependency,
  removeDependency,
  validateDependenciesCommand as validateDependencies,
  fixDependenciesCommand as fixDependencies
...
function registerCommands(programInstance) {
  ...
  programInstance
    ...
    .option('-o, --output <path>', 'Output file path', 'scripts/task-complexity-report.json')
    .option('-m, --model <model>', 'Override LLM model to use')
    .option('-t, --threshold <number>', 'Minimum score for expansion recommendation', '5')
    .option('-f, --file <path>', 'Path to tasks file', 'tasks/tasks.json')
    .option('-r, --research', 'Use Perplexity AI for research-backed analysis', false)
    .action(async (options) => {
      try {
        await processTaskComplexity(options);
      } catch (error) {
  ...
  programInstance
    ...
    .option('-f, --file <file>', 'Path to the tasks file', 'tasks/tasks.json')
    .action(async (options) => {
      await validateDependencies(options.file);
    });
  ...
  programInstance
    ...
    .option('-f, --file <file>', 'Path to the tasks file', 'tasks/tasks.json')
    .action(async (options) => {
      await fixDependencies(options.file);
    });
